    ...
    <div v-if="viewTransactions" class="flex flex-col grow overflow-hidden">
      ...
      <div class="flex items-center h-20 mb-4 py-3 px-3 border-y border-gray-200 bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
        <div v-if="assetData" class="flex justify-between w-full">
          <div>
            <p class="mr-2 font-normal text-2xl tracking-wider truncate">{{ $formatNumber(assetData.current_price, assetData.current_price < 10 ? 3 : 2, true, false, assetData.currency_symbol) }}</p>
...
export default defineComponent({
  ...
  methods: {
    ...
    setActiveTab(newTab: string) {
      ...
      this.utilityStore.logUserActivity(109, "Transactions Page", "INFO", `User switched to the TRANSACTIONS ${newTab} tab.`)
    },

<template>
  <div v-if="show" class="flex flex-col grow overflow-hidden">
            ...
            <div class="mb-3">
              <div class="flex justify-end">
                <div class="grow">
                  ...
                  <p class="text-tiny my-0.5 text-gray-200">Price: {{ $formatNumber(holding.current_price, holding.current_price < 10 ? 3 : 2, true, false, holding.currency_symbol) }}</p>
                </div>
                <div class="w-20 text-right mt-0.5 ml-2 font-normal">
                  ...
                  <p class="text-tiny text-gray-200" :class="$fontSize($formatNumber(holding.initial_value, 2, true, false), 'text-tiny', 15)">{{ $formatNumber(holding.initial_value, 2, true, false) }}</p>
                </div>
                ...
                <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': BigNumber(holding.current_value).minus(holding.initial_value).isLessThan(-0.0000000001), 'text-bright-green': BigNumber(holding.current_value).minus(holding.initial_value).isGreaterThan(0.0000000001) }">
                  ...
                  <p :class="$fontSize($formatNumber(BigNumber(holding.current_value).minus(holding.initial_value).div(holding.initial_value).times(100), 2, false, true) + '%', 'text-tiny')">{{ holding.initial_value > 0 ? $formatNumber(BigNumber(holding.current_value).minus(holding.initial_value).div(holding.initial_value).times(100), 2, false, true) : '0.00' }}%</p>
                </div>
              </div>

              <p v-if="holding.current_quantity > 0" class="text-tiny text-gray-200">Cost: {{ $formatNumber(holding.avg_initial_price, holding.avg_initial_price < 10 ? 3 : 2, true, false, holding.currency_symbol) }} ({{ $formatNumber(holding.initial_value / holding.current_quantity, holding.initial_value / holding.current_quantity < 10 ? 3 : 2, true, false) }}) x {{ $formatNumber(holding.current_quantity, holding.current_quantity < 10 ? 3 : 2) }} shares</p>

              <div v-if="new BigNumber(holding.all_time_initial).isGreaterThan(holding.initial_value) || holding.realized > 0">
                <p class="my-0.5 text-tiny">Realized: <span class="font-normal" :class="{ 'text-bright-red': holding.realized < 0, 'text-bright-green': holding.realized > 0 }">{{ $formatNumber(holding.realized, 2, true, true) }} ({{ holding.realized_initial > 0 ? $formatNumber(BigNumber(holding.realized).div(holding.realized_initial).times(100), 2, false, true) : '0.00' }}%)</span></p>
                <p class="my-0.5 text-tiny h-4">All-time: <span class="font-normal" :class="{ 'text-bright-red': BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value).isLessThan(0), 'text-bright-green': BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value).isGreaterThan(0) }">{{ $formatNumber(BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value), 2, true, true) }} ({{ holding.all_time_initial > 0 ? $formatNumber(BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value).div(holding.all_time_initial).times(100), 2, false, true) : '0.00' }}%)</span></p>
              </div>
            </div>
          </NuxtLink>
        </div>
    ...
    <div v-if="holdings != null && holdings.length > 0" class="py-2 px-3 border-t border-gray-100 bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      ...
      <div v-if="total.all_time_initial.isGreaterThan(total.initial_value) || total.realized.isGreaterThan(0)">

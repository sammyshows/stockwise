<template>
  <div class="page-background w-full h-full flex flex-col justify-between pt-10 pb-5 px-10 overflow-scroll">
    ...
    <div>
      <!--      currently a bug causes the message below to appear everytime      -->
<!--      <p v-if="errorMessage === 'error'" class="mt-2.5 text-xs text-center text-red-600">&#10033;&nbsp;&nbsp;An error has occurred. Please verify that the details you entered are correct.</p>-->
      ...
      <div class="mt-2.5">
        ...
        <input @keyup="validatePassword(); invalid.password = false;" v-model="password" autocomplete="on" placeholder="Password" type="text" class="placeholder:text-gray-500 w-full h-12 mt-1.5 text-sm rounded-xl bg-gray-900/20 border border-gray-400/40 focus:ring-0 focus:border-white">
      </div>

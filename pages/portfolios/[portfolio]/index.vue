        ...
        <div class="mb-3">
          <div class="flex justify-end">
            ...
            <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': holding.daily_change < 0, 'text-bright-green': holding.daily_change > 0 }">
              ...
              <p class="text-tiny">{{ $addSign($formatNumber(BigNumber(holding.daily_change).div(BigNumber(holding.current_value).minus(holding.daily_change)).times(100), 2)) }}%</p>
            </div>
            <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': BigNumber(holding.current_value).minus(holding.initial_value) < 0, 'text-bright-green': BigNumber(holding.current_value).minus(holding.initial_value) > 0 }">
              <p class="h-5 text-xs">{{ $addSign($formatNumber(BigNumber(holding.current_value).minus(holding.initial_value), 2)) }}</p>
              <p class="text-tiny">{{ $addSign($formatNumber(BigNumber(holding.current_value).minus(holding.initial_value).div(holding.initial_value).times(100), 2)) }}%</p>
            </div>
          </div>
          ...
          <div v-if="holding.realized">
            ...
            <p class="font-light text-tiny h-4">All-time: <span class="font-normal" :class="{ 'text-bright-red': BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value).isLessThan(0), 'text-bright-green': BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value).isGreaterThan(0) }">{{ $addSign($formatNumber(BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value), 2)) }} ({{ $addSign($formatNumber(BigNumber(holding.realized).plus(holding.current_value).minus(holding.initial_value).div(holding.all_time_initial).times(100), 2)) }}%)</span></p>
          </div>
        </div>
      </NuxtLink>
    </div>

        ...
        <div v-for="holding in holdings" :key="holding.holding_id">
          <NuxtLink :to="{ name: 'portfolios-portfolio-holdings-holding', params: { portfolio: $route.params.portfolio, holding: holding.holding_id } }">
            <div class="mb-3">
              ...
              <p class="text-tiny text-gray-200">Cost: {{ $formatNumber(holding.avg_initial_price, 2, true, false, holding.currency_symbol) }} ({{ $formatNumber(holding.initial_value / holding.current_quantity, 2, true, false) }}) x {{ $formatNumber(holding.current_quantity, 2) }} shares</p>
    ...
    <div v-if="holdings != null && holdings.length > 0" class="py-2 px-3 border-t border-gray-100 bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      <div class="flex justify-end">
        <div class="grow">
          ...
          <p class="font-light text-tiny my-0.5 text-gray-200">{{ holdings.length }} HOLDINGS</p>
        </div>
        <div class="w-20 text-right mt-0.5 ml-2 font-normal">

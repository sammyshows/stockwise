    ...
    <div v-if="transactions && transactions.length > 0" class="overflow-scroll grow px-3">
      <TransitionGroup tag="div" name="form">
        <div v-for="transaction in transactions" :key="transaction.transaction_id">
          <NuxtLink :to="{ name: 'portfolios-portfolio-holdings-holding-transactions-transaction', params: { portfolio: $route.params.portfolio, holding: $route.params.holding, transaction: transaction.transaction_id } }">
            <!--      STOCKS or FOREX      -->
            <div v-if="transaction.asset_type !== 2" class="mb-3">
              <div v-if="transaction.type === 0" class="flex">
                ...
                <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': transaction.total_change < -0.0000000001, 'text-bright-green': transaction.total_change > 0.0000000001 }">
                  ...
                  <p :class="$fontSize(transaction.current_quantity > 0 ? $formatNumber(BigNumber(transaction.total_change).div(transaction.initial_value).times(100), 2, false, true) + '%' : '0.00', 'text-tiny')">{{ transaction.initial_value > 0 ? $formatNumber(BigNumber(transaction.total_change).div(transaction.initial_value).times(100), 2, false, true) : '0.00' }}%</p>
                </div>
              </div>

              <div v-else-if="transaction.type === 1" class="grid grid-cols-12">
              ...
              <div v-if="transaction.type === 3" class="flex">
                <div class="grow">
                  <h2 class="h-5 w-28 text-sm font-bold tracking-wider truncate">{{ $formatNumber(transaction.initial_quantity, 3) }}</h2>
                  <p class="font-light text-tiny my-0.5 text-gray-200">@ {{ $formatNumber(transaction.price, 3, true, false, transaction.currency_symbol) }} per share</p>
                </div>
                <div class="w-20 text-right mt-0.5 ml-2 font-normal">
                  <p class="h-5" :class="$fontSize($formatNumber(transaction.current_value, 2, true, false), 'text-xs', 12)">{{ $formatNumber(transaction.current_value, 2, true, false) }}</p>
                  <p class="text-gray-200" :class="$fontSize($formatNumber(transaction.initial_value, 2, true, false), 'text-tiny', 15)">{{ $formatNumber(0.00, 2, true, false) }}</p>
                </div>
                <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': transaction.daily_change < 0, 'text-bright-green': transaction.daily_change > 0 }">
                  <p class="h-5" :class="$fontSize($formatNumber(transaction.daily_change, 2, false, true), 'text-xs')">{{ $formatNumber(transaction.daily_change, 2, false, true) }}</p>
                  <p :class="$fontSize(transaction.current_quantity > 0 ? $formatNumber(transaction.daily_percent, 2, false, true) + '%' : '0.00', 'text-tiny')">{{ transaction.current_quantity > 0 ? $formatNumber(transaction.daily_percent, 2, false, true) : '0.00' }}%</p>
              ...
              <div v-if="transaction.initial_quantity > transaction.current_quantity">
                <p v-if="transaction.current_quantity > 0" class="text-tiny text-gray-200">Remaining quantity: {{ $formatNumber(transaction.current_quantity, 2) }} shares</p>
                <p class="mb-0.5 font-light text-tiny">Realized: <span :class="{ 'text-bright-red': BigNumber(transaction.realized).isLessThan(0), 'text-bright-green': BigNumber(transaction.realized).isGreaterThan(0) }">{{ $formatNumber(transaction.realized, 2, true, true) }} ({{ transaction.realized_initial > 0 ? $formatNumber(BigNumber(transaction.realized).div(transaction.realized_initial).times(100), 2, false, true) : '0.00' }}%)</span></p>
<!--                <p class="my-0.5 font-light text-tiny">All-time: <span :class="{ 'text-bright-red': parseFloat(transaction.realized) + parseFloat(transaction.total_change) < 0, 'text-bright-green': parseFloat(transaction.realized) + parseFloat(transaction.total_change) > 0 }">{{ $formatNumber(BigNumber(transaction.realized).plus(transaction.total_change), 2, true, true) }} ({{ $formatNumber(BigNumber(transaction.realized).plus(transaction.total_change).div(transaction.all_time_initial).times(100), 2, false, true) }}%)</span></p>-->
            ...
            <!--      CASH      -->
            <div v-else-if="transaction.asset_type === 2" class="mb-3">
              <div v-if="transaction.type === 0" class="flex">
                <div class="grow">
                  <h2 class="h-5 w-28 text-sm font-bold tracking-wider truncate">{{ $formatNumber(transaction.initial_quantity, 3) }}</h2>
                  <p class="font-light text-tiny my-0.5 text-gray-200">@ {{ $formatNumber(new BigNumber(transaction.current_value).div(transaction.current_quantity), 3, true, false) }}</p>
                  <p class="font-light text-teeny my-0.5 text-gray-200">{{ new Date(transaction.datetime).toDateString().slice(3)}} - BUY</p>
                </div>
                <div class="w-20 text-right mt-0.5 ml-2 font-normal">
                  <p class="h-5" :class="$fontSize($formatNumber(transaction.current_value, 2, true, false), 'text-xs', 12)">{{ $formatNumber(transaction.current_value, 2, true, false) }}</p>
    ...
    <div v-if="transactions[0]?.asset_type !== 2 && transactions != null && transactions.length > 0" class="py-2 px-3 border-t border-gray-100 bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      <div class="flex justify-end">
        ...
        <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': total.current_value.minus(total.initial_value).isLessThan(0), 'text-bright-green': total.current_value.minus(total.initial_value).isGreaterThan(0) }">
          ...
          <p :class="$fontSize($formatNumber(total.current_value.minus(total.initial_value).div(total.initial_value).times(100), 2, false, true), 'text-tiny')">{{ total.initial_value > 0 ? $formatNumber(total.current_value.minus(total.initial_value).div(total.initial_value).times(100), 2, false, true) : '0.00' }}%</p>
        </div>
      </div>
      <p v-if="total.current_quantity.isGreaterThan(0)" class="font-light text-tiny mb-0.5 text-gray-200">UNREALIZED: {{ $formatNumber(total.current_quantity, 2) }} SHARES @ {{ $formatNumber(total.initial_value_unexchanged.div(total.current_quantity), 3, true, false, transactions[0].currency_symbol) }} ({{ $formatNumber(total.initial_value.div(total.current_quantity), 3, true, false) }})</p>
      <div v-if="total.realized.isGreaterThan(0) || total.dividends.toNumber()">
        ...
        <p class="text-tiny my-0.5">All-time: <span class="font-normal" :class="{ 'text-bright-red': total.realized.plus(total.current_value).minus(total.initial_value) < 0, 'text-bright-green': total.realized.plus(total.current_value).minus(total.initial_value) > 0 }">{{ $formatNumber(total.realized.plus(total.current_value).minus(total.initial_value), 2, true, true) }} ({{ total.all_time_initial > 0 ? $formatNumber(total.realized.plus(total.current_value).minus(total.initial_value).div(total.all_time_initial).times(100), 2, false, true) : '0.00' }}%)</span></p>
        <p v-if="isDividend" class="text-tiny my-0.5">Dividends: <span class="font-normal text-bright-green">{{ $formatNumber(total.dividends, 2, true, true) }}</span></p>
      </div>
    </div>

    <div v-else-if="transactions[0]?.asset_type === 2 && transactions != null && transactions.length > 0" class="py-2 px-3 border-t border-gray-100 bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      <div class="flex justify-end">
        <div class="grow">
          <h2 class="text-sm font-bold tracking-wider truncate">Summary</h2>
          <p class="font-light text-tiny my-0.5 text-gray-200">{{ transactions.length }} TRANSACTIONS</p>
        ...
        <div class="w-20 text-right mt-0.5 ml-2 font-normal">
          <p class="h-5" :class="$fontSize($formatNumber(total.current_value, 2, true, false), 'text-xs', 12)">{{ $formatNumber(total.current_value, 2, true, false) }}</p>
          <p class="text-gray-200" :class="$fontSize($formatNumber(total.initial_value, 2, true, false), 'text-tiny', 15)">{{ $formatNumber(total.current_value, 2, true, false) }}</p>
        </div>
        <div class="w-16 text-right mt-0.5 ml-2 font-normal">
          <p class="h-5 text-xs">0.00</p>
          <p class="text-tiny">0.00%</p>
        </div>
        <div class="w-16 text-right mt-0.5 ml-2 font-normal">
      ...
      <p v-if="total.current_quantity.isGreaterThan(0)" class="font-light text-tiny mb-0.5 text-gray-200">UNREALIZED: {{ $formatNumber(total.current_quantity, 2) }} @ {{ $formatNumber(transactions[0].price, 3, true, false, transactions[0].currency_symbol) }} ({{ $formatNumber(total.current_value.div(total.current_quantity), 3, true, false) }})</p>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  computed: {
    isDividend() {
      return this.transactions.find((transaction) => transaction.type === 2 || transaction.type === 3)
    }
  },

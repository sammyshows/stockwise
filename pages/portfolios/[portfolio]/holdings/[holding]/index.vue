    ...
    <NuxtLink v-for="transaction in transactions" :to="{ name: 'portfolios-portfolio-holdings-holding-transactions-transaction', params: { portfolio: $route.params.portfolio, holding: $route.params.holding, transaction: transaction.id, assetName: transaction.name, assetSymbol: transaction.symbol + ' : ' + transaction.exchange, assetName: transaction.name } }">
      <div class="flex justify-end">
        ...
        <div class="w-20 text-right mt-0.5 ml-2">
          <p class="h-5 text-xs font-light">A${{ $formatNumber(transaction.current_value, 2) }}</p>
        ...
        <div class="w-16 text-right mt-0.5 ml-2" :class="{ 'text-bright-red': transaction.daily_change < 0, 'text-bright-green': transaction.daily_change > 0 }">
          <p class="h-5 text-xs font-light">{{ $addSign($formatNumber(transaction.daily_change, 2)) }}</p>
        ...
        <div class="w-16 text-right mt-0.5 ml-2" :class="{ 'text-bright-red': transaction.total_change < 0, 'text-bright-green': transaction.total_change > 0 }">
          <p class="h-5 text-xs font-light">{{ $addSign($formatNumber(transaction.total_change, 2)) }}</p>
      ...
      <p class="font-light text-tiny h-4">All-time: <span class="text-bright-green">{{ $addSign($formatNumber(transaction.total_change, 2)) }}({{ $addSign($formatNumber(transaction.total_change / transaction.initial_value * 100, 2)) }}%)</span></p>

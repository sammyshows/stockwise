<template>
  <div class="overflow-hidden flex flex-col grow">
    ...
    <div class="overflow-scroll grow">
      <NuxtLink v-for="transaction in transactions" :to="{ name: 'portfolios-portfolio-holdings-holding-transactions-transaction', params: { portfolio: $route.params.portfolio, holding: $route.params.holding, transaction: transaction.id, assetName: transaction.name, assetSymbol: transaction.symbol + ' : ' + transaction.exchange, assetName: transaction.name } }">
        <div class="flex justify-end">
          ...
          <div class="w-16 text-right mt-0.5 ml-2 font-medium" :class="{ 'text-bright-red': transaction.daily_change < 0, 'text-bright-green': transaction.daily_change > 0 }">
          ...
          <div class="w-16 text-right mt-0.5 ml-2 font-medium" :class="{ 'text-bright-red': transaction.total_change < 0, 'text-bright-green': transaction.total_change > 0 }">
        ...
        <p class="font-light text-tiny h-4">All-time: <span class="font-medium" :class="{ 'text-bright-red': transaction.total_change < 0, 'text-bright-green': transaction.total_change > 0 }">{{ $addSign($formatNumber(transaction.total_change, 2)) }}({{ $addSign($formatNumber(transaction.total_change / transaction.initial_value * 100, 2)) }}%)</span></p>
    ...
    <div v-if="transactions != null && transactions.length > 0" class="py-2 border-t border-gray-300" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      ...
      <p class="text-tiny my-0.5 text-gray-300">Realised: <span class="font-medium" :class="{ 'text-bright-red': total.current_value - total.initial_value < 0, 'text-bright-green': total.current_value - total.initial_value > 0 }">+182.82(+47.98%)</span></p>
    </div>
  </div>
</template>

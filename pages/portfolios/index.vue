            ...
            <div class="mb-3">
              <div class="flex justify-end">
                ...
                <div class="w-20 text-right mt-0.5 ml-2 font-normal">
                  <p class="h-5" :class="$fontSize($formatNumber(portfolio.current_value, 2, true, false) || $formatNumber('0', 0, true, false), 'text-xs', 13)">{{ $formatNumber(portfolio.current_value, 2, true, false) || $formatNumber('0', 0, true, false) }}</p>
                  <p class="text-gray-300" :class="$fontSize($formatNumber(portfolio.initial_value, 2, true, false) || $formatNumber('0', 0, true, false), 'text-tiny', 16)">{{ $formatNumber(portfolio.initial_value, 2, true, false) || $formatNumber('0', 0, true, false) }}</p>
                </div>
              ...
              <div v-if="portfolio.realized">
                <p class="h-4 text-tiny">Realized: <span class="font-normal" :class="{ 'text-bright-red': portfolio.realized < 0, 'text-bright-green': portfolio.realized > 0 }">{{ $formatNumber(portfolio.realized, 2, false, true) }} ({{ $formatNumber(BigNumber(portfolio.realized).div(portfolio.realized_initial).times(100), 2, false, true) }}%)</span></p>
    ...
    <div v-if="total" class="py-2 px-3 border-t border-gray-300 bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      <div class="flex justify-end">
        <div class="grow">
          ...
          <div class="w-20 text-right mt-0.5 ml-2 font-normal">
          <p class="h-5" :class="$fontSize($formatNumber(total.current_value, 2, true, false) || $formatNumber('0', 0, true, false), 'text-xs', 13)">{{ $formatNumber(total.current_value, 2, true, false) || $formatNumber('0', 0, true, false) }}</p>
          <p class="text-gray-300" :class="$fontSize($formatNumber(total.initial_value, 2, true, false) || $formatNumber('0', 0, true, false), 'text-tiny', 16)">{{ $formatNumber(total.initial_value, 2, true, false) || $formatNumber('0', 0, true, false) }}</p>
        </div>
        <div class="w-16 text-right mt-0.5 ml-2 font-normal" :class="{ 'text-bright-red': total.daily_change < 0, 'text-bright-green': total.daily_change > 0 }">
          ...
          <p :class="$fontSize($formatNumber(total.daily_change.div(BigNumber(total.current_value).minus(total.daily_change)).times(100), 2, false, true) || '-', 'text-tiny')">{{ $formatNumber(total.daily_change.div(BigNumber(total.current_value).minus(total.daily_change)).times(100), 2, false, true) || '-' }}<span v-if="$formatNumber(total.daily_change.div(BigNumber(total.current_value).minus(total.daily_change)).times(100), 2, false, true)">%</span></p>
        </div>

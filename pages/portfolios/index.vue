<template>
  <div v-if="show" class="overflow-hidden flex flex-col grow">
    ...
    <div v-if="portfolios" class="overflow-scroll grow px-3">
      <TransitionGroup tag="div" name="form">
        <div v-for="portfolio in portfolios" :key="portfolio.portfolio_id">
          <NuxtLink :to="{ name: 'portfolios-portfolio', params: { portfolio: portfolio.portfolio_id } }">
            <div class="mb-3">
              <div class="flex justify-end">
                <div class="grow">
                  <h2 class="h-5 w-28 text-sm font-bold tracking-wider truncate">{{ portfolio.portfolio_name }}</h2>
                  <p class="font-light text-tiny my-0.5 text-gray-200">{{ portfolio.transaction_count }} TRANSACTIONS</p>
                </div>
    ...
    <div v-if="total" class="py-2 px-3 border-t bg-gray-900/30" style="box-shadow: 0 -5px 25px -20px rgb(75 85 99);">
      <div class="flex justify-end">
        <div class="grow">

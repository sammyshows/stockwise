          ...
          <div class="modal-background relative inline-block align-bottom rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            <div class="sm:flex sm:items-start">
              ...
              <div v-if="typeToConfirm">
                ...
                <input @keyup="checkMatch($event)" type="text" placeholder="Please type your email to confirm" class="w-full mt-4 py-2.5 text-xs rounded-md bg-gray-900/20 border border-red-400/40 focus:ring-0 focus:border-red-500" style="touch-action: manipulation">
              </div>
            </div>
...
export default {
  ...
  // this component was fkn bullshit to make work. If you cleanup be sure to test.
  data() {
    return {
  ...
  mounted() {
    this.disableDelete = this.typeToConfirm !== ''
  },

  watch: {
    typeToConfirm(string) {
      this.disableDelete = string !== ''
    }
  },

  methods: {
    checkMatch(event): void {
      if (this.typeToConfirm !== '')
        this.disabledDelete = event.target.value.trim() !== this.typeToConfirm
      else

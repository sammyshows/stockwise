...
CREATE TABLE holdings (id INT GENERATED ALWAYS AS IDENTITY, PRIMARY KEY(id), portfolio_id INT, asset_id INT, CONSTRAINT fk_portfolio FOREIGN KEY(portfolio_id) REFERENCES portfolios(id), CONSTRAINT fk_asset FOREIGN KEY(asset_id) REFERENCES assets(id));
INSERT INTO holdings (portfolio_id, asset_id) VALUES (1, 1);
INSERT INTO holdings (portfolio_id, asset_id) VALUES (1, (SELECT id FROM assets WHERE symbol = 'tsla'));
INSERT INTO holdings (portfolio_id, asset_id) VALUES (2, 1);
INSERT INTO holdings (portfolio_id, asset_id) VALUES (2, 3);
INSERT INTO holdings (portfolio_id, asset_id) VALUES (3, 2);
INSERT INTO holdings (portfolio_id, asset_id) VALUES (3, 3);
INSERT INTO holdings (portfolio_id, asset_id) VALUES (3, 4);

CREATE TABLE transactions (id INT GENERATED ALWAYS AS IDENTITY, PRIMARY KEY(id), holding_id INT, type TEXT, quantity NUMERIC(10,3), initial_price NUMERIC(10,3), date DATE NOT NULL DEFAULT CURRENT_DATE, exchange_rate NUMERIC(10,3), CONSTRAINT fk_holding FOREIGN KEY(holding_id) REFERENCES holdings(id) ON DELETE CASCADE);
...
CREATE FUNCTION uspReadTransactions(holding_id INT) RETURNS TABLE (id INT, symbol TEXT, exchange TEXT, name TEXT, shares NUMERIC(50,50), price NUMERIC(50,50), initial_value NUMERIC(10,3), current_value NUMERIC (10,3), total_percent NUMERIC (10,2), daily_value NUMERIC(10,3), daily_percent NUMERIC(10,2)) LANGUAGE plpgsql AS $$ DECLARE portfolio RECORD;
BEGIN
    RETURN QUERY

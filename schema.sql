...
CREATE TABLE assets (id INT GENERATED ALWAYS AS IDENTITY, PRIMARY KEY(id), current_price NUMERIC, prev_close NUMERIC, symbol TEXT UNIQUE, name TEXT, exchange TEXT);
...
CREATE TABLE transactions (id INT GENERATED ALWAYS AS IDENTITY, PRIMARY KEY(id), holding_id INT, type TEXT, quantity NUMERIC, initial_price NUMERIC, date DATE NOT NULL DEFAULT CURRENT_DATE, exchange_rate NUMERIC, CONSTRAINT fk_holding FOREIGN KEY(holding_id) REFERENCES holdings(id) ON DELETE CASCADE);
...
CREATE FUNCTION uspReadPortfolios() RETURNS TABLE (id INT, name VARCHAR(50), transaction_count BIGINT, initial_value NUMERIC, current_value NUMERIC, total_percent NUMERIC (10,2), daily_value NUMERIC, daily_percent NUMERIC(10,2)) LANGUAGE plpgsql AS $$ DECLARE portfolio RECORD;
...
CREATE FUNCTION uspReadTransactions(holding_id INT) RETURNS TABLE (id INT, symbol TEXT, exchange TEXT, name TEXT, shares NUMERIC, price NUMERIC, initial_value NUMERIC, current_value NUMERIC, total_percent NUMERIC (10,2), daily_value NUMERIC, daily_percent NUMERIC(10,2)) LANGUAGE plpgsql AS $$ DECLARE portfolio RECORD;
BEGIN
    RETURN QUERY
